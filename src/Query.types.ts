...
export interface TTableQueryState {
...
export interface TCreateTableQueryParams<
...
> {
  ...
  readonly state: TTableQueryState;
  // parent queries
  readonly dependencies: Array<TTableQueryDependency>;
}

export interface TTableQueryDependency {
  // table or cte name
  readonly from: Ast.Identifier;
  // The current query state (select, filters, order, etc.)
  readonly state: TTableQueryState;
...
export interface TTableQueryInternal<
  InCols extends ExprRecordNested,
  OutCols extends ExprRecord,
> extends TTableQueryDependency {
  readonly inputColsRefs: InCols;
  // Identifiers of the columns of the current query
  readonly outputColsRefs: OutCols;
  // Selected expressions of the current query
...
> {
  ...
  readonly [PRIV]: TTableQueryInternal<InCols, OutCols>;
   ...
   */
  where(whereFn: ColsFn<InCols, TExprUnknow>): TTableQuery<InCols, OutCols>;

  /**
   * Add conditions to the WHERE clause using en AND operator
   * @param whereFn An expression function that should return an expression
   */
  andWhere(whereFn: ColsFn<InCols, TExprUnknow>): TTableQuery<InCols, OutCols>;
   ...
   */
  having(havingFn: ColsFn<InCols, TExprUnknow>): TTableQuery<InCols, OutCols>;
   ...
   */
  andSortAsc(exprFn: ColsFn<InCols, TExprUnknow>): TTableQuery<InCols, OutCols>;
   ...
   */
  innerJoin<RTable extends TTableQuery<any, any>, Alias extends string>(
    table: RTable,
    alias: Alias,
   ...
   * @param joinOn An expression function that should return a boolean expression that will be used to join the tables
   */
  leftJoin<RTable extends TTableQuery<any, any>, Alias extends string>(
    table: RTable,
    alias: Alias,
   ...
   */
  all(): TQueryOperation<Array<Prettify<ExprRecordOutput<OutCols>>>>;
   ...
   */
  maybeOne(): TQueryOperation<Prettify<ExprRecordOutput<OutCols>> | null>;

  /**
   * Return an Database operation that will execute the query and return a single result
   * If results count is not exactly 1, will throw
   */
  one(): TQueryOperation<Prettify<ExprRecordOutput<OutCols>>>;
   ...
   * If no result is found, will return null
   */
  maybeFirst(): TQueryOperation<Prettify<ExprRecordOutput<OutCols>> | null>;
